...
import { Menu, X, GraduationCap, Home, Map, BookOpen, Users, Info, Briefcase, FlaskConical, LogOut } from "lucide-react";
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.path} to={item.path}>
                  <Button variant={isActive ? "gradient" : "ghost"} size="sm" className={`gap-2 ${isActive ? "" : "text-foreground/70 hover:text-foreground"}`}>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link key={item.path} to={item.path} onClick={() => setIsOpen(false)}>
                    <Button variant={isActive ? "gradient" : "ghost"} className="w-full justify-start gap-3">
